...
import static org.factory.factory.GameHandler.FactoryItem.ProcessItemMeta;
...
public class TraderManager {
    ...
    public static void OpenQuarterMaster(Player player, String mineral){
        Merchant merchant = Bukkit.createMerchant(sendText("&nQuarter Master"));

        List<MerchantRecipe> trades = new ArrayList<>();

//            List<String> mineralList = Arrays.asList
//            (
...
//            );
        ...
        List<String> materialList2 = Arrays.asList("sword", "hammer", "bow", "gun");

        int equipmentPendant = 1;
        int equipmentPendantAmount = 3;
        int sort = 0;

        for (String materialType : materialList){
            MerchantRecipe merchantRecipe = new MerchantRecipe(new ItemStack(GetItem(mineral+materialType)), 9999);
            ItemStack pendant = new ItemStack((new ItemStack(GetItem("dungeonpendanttier"+equipmentPendant))));
            pendant.setAmount(equipmentPendantAmount);
            merchantRecipe.addIngredient(pendant);
            trades.add(merchantRecipe);
        }
        for (String materialType : materialList){
            MerchantRecipe merchantRecipe = new MerchantRecipe(new ItemStack(GetItem(mineral+materialType+"2")), 9999);
            ItemStack pendant = new ItemStack((new ItemStack(GetItem("dungeonpendanttier"+equipmentPendant))));
            pendant.setAmount(equipmentPendantAmount);
            merchantRecipe.addIngredient(pendant);
            ItemStack previousEquipment = new ItemStack(ProcessItemMeta(new ItemStack(GetItem(mineral+materialType))));
            merchantRecipe.addIngredient(previousEquipment);
            trades.add(merchantRecipe);
        }

        for (String materialType : materialList2){
            MerchantRecipe merchantRecipe = new MerchantRecipe(new ItemStack(GetItem(mineral+materialType)), 9999);
            ItemStack pendant = new ItemStack((new ItemStack(GetItem("dungeonpendanttier"+equipmentPendant))));
            pendant.setAmount(equipmentPendantAmount);
            merchantRecipe.addIngredient(pendant);
